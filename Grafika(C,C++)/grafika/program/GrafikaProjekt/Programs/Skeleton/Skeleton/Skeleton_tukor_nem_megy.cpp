...
const char* fragmentSource = R"(
	...
	void getObjPlane(int i, out vec3 p, out vec3 normal, out vec3 center){
		vec3 p1 = dV[dP[i][0]-1], p2 = dV[dP[i][2]-1], p3 = dV[dP[i][3]-1];
	...
	Hit firstIntersect(Ray ray) {
		...
		bestHit = intersectDodecahedronAndPortal(ray,bestHit,0,0.98);
	...
	vec3 rotate(vec3 position, vec3 normal, vec3 center, int degree){
		// vec4 n = vec4(position,1);
		// vec4 res = rotationMatrix(normal, radians(degree))*n;
		// return vec3(res.x,res.y,res.z);
		vec3 v = position-center;
		vec3 k = normalize(normal);
		float r = radians(degree);
		return center+(v*cos(r)+(cross(k,v)*sin(r))+(k*dot(k,v)*(1-cos(r))));
...
public:
	void build() {
		...
		lights.push_back(new Light(vec3(1, 1, 1), vec3(3, 3, 3), vec3(0.6f, 0.6f, 0.9f)));

		vec3 kd(0.7f, 0.2f, 0.1f), ks(10, 10, 10);
