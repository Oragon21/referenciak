...
const char* fragmentSource = R"(
	...
	Hit solveQuadratic(float a, float b, float c, Ray ray, Hit hit, float zmin, float zmax, float normz){
		...
		if(discr>=0){
			...
			if(t1>0 && (t1<hit.t || hit.t < 0)) {
				...
				hit.normal = normalize(vec3(2*-hit.position.x, -hit.position.y, normz));
				hit.mat=1;
			}
		}
		return hit;
	...
	vec3 rotate(vec3 position, vec3 center, vec3 normal, float degree){
		...
		vec3 k = normal; // already normalized in getPentagonPlane 
...
class Scene {
	...
	const float g = 0.618f, G = 1.618f; // btcaux=exp(-8.8)*S2F^3.3
